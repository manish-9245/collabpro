  ...
  async handle(path: string, args: any, authUserEmail: string | null, ipAddress: string): Promise<any> {
    ...
    switch (path) {
      ...
      case 'user:createUser': {
        ...
        result = await this.prisma.user.upsert({
          where: { email },
          update: { name, image },
      ...
      case 'user:getUserProfile': {
        ...
        if (profile.isPrivate && requesterEmail && requesterEmail !== email) {
          ...
          const requesterCreatedTeams = await this.prisma.team.findMany({
            where: { createdBy: requesterEmail },
          });
          const requesterTeamIds = [...requesterTeams.map((t: any) => t.teamId), ...requesterCreatedTeams.map((t: any) => t.id)];

          const userTeams = await this.prisma.teamMember.findMany({
            where: { userEmail: email },
          });
          const userCreatedTeams = await this.prisma.team.findMany({
            where: { createdBy: email },
          });
          const userTeamIds = [...userTeams.map((t: any) => t.teamId), ...userCreatedTeams.map((t: any) => t.id)];
