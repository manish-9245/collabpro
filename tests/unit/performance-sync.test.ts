...
describe('GrahakAI Performance & Sync Engine Tests', () => {
  ...
  describe('Issue #142: Last-Writer-Wins Data Loss Prevention (State Merging)', () => {
    it('should concurrently merge document block updates rather than blindly overwriting', async () => {
      ...
      // Verify the compare-and-swap update was called with merged blocks
  ...
  describe('Issue #197 (partial): compare-and-swap replaces the debounce map', () => {
    ...
    // compare-and-swap (read, merge, `updateMany` gated on an unchanged
    // document, retry on conflict) pattern already used by
    // `collabpro_update_document`. A single, uncontested request now does
    // exactly one findUnique + one updateMany, no debounce delay.
    //
    ...
    // mock) proving both writers' changes converge instead of one being
    // silently dropped.
    it('performs a single read-merge-CAS write with no debounce delay for an uncontested request', async () => {
      mockFileFindUnique.mockResolvedValue({
      ...
      expect(savedDoc.blocks.map((b: any) => b.id)).toEqual(['existing', 'new-block']);
    });
  });
});
